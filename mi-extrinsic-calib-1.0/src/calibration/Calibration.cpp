...
namespace perls
{
    Calibration::Calibration (std::string configFile)
    {
        ...
        //load scans
        char *scan_folder = "/home/phyorch/PROJECT/mi-extrinsic-calib-1.0/data/scans";
...
//        free (use_scans);
    ...
    Calibration::load_image (int imageIndex)
    {
        ...
        std::string index = std::to_string(imageIndex);
        std::string imageName;
        std::string image_type = ".ppm";
        std::string data_folder = "/home/phyorch/PROJECT/mi-extrinsic-calib-1.0/data/";

        Image_t currImage;
        for (int i = 0; i < this->m_NumCams; i++)
        {
            std::stringstream inter;
            std::string image_folder = data_folder + "Cam" + std::to_string(i) + "/";
            inter << std::setw(4) << std::setfill('0') << index;
            inter >> index;
            image_base_name = "image" + index;
            imageName = image_folder + image_base_name + image_type;
            printf ("Loading file-: %s\n",imageName.data());

            IplImage* iplimage = cvLoadImage (imageName.data(), 0); //load image as grayscale.
            if (iplimage == NULL)
            {
    ...
    Calibration::mi_cost (ssc_pose_t x0)
    {
        ...
        for (int i=0;i<256;i++)
        {
            if (prob.refcProb.at<float>(i)==0)
            {
                prob.refcProb.at<float>(i) = 1.0e-12;
            }
            for (int j=0;j<256;j++)
            {
                if (prob.jointProb.at<float>(i,j)==0)
                {
                    prob.jointProb.at<float>(i,j) = 1.0e-12;
                }
            }
        }
